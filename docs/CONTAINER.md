...
| Sample rate / layout | V1 reference writer (`src/dmxreplay/container/writer.py`) re-encodes to 48 kHz, mono or stereo (source channel count ≥ 2 becomes stereo, else mono) — not a format requirement, just this implementation's choice; a reader MUST use the manifest's `audio.sample_rate`/`audio.channels`, never assume 48 kHz. |
| Timing constraint | An audio *stream* can only be declared (`add_stream`) before the container header is written, same as the manifest attachment (§4) — practically, this means a DMXReplay writer must have the **complete** audio source available up front; there is no way to append/stream audio into an already-open file the way DMX video frames arrive live one at a time. V1's reference `Recorder` therefore has no live audio *capture* path; attaching a pre-existing audio file to an already-recorded show is `dmxreplay-convert --add-audio`'s job (docs/API.md §7) instead. |
| Ordering with the manifest attachment | The manifest attachment (§4) MUST be added to the container **before** any audio (or video) packet is muxed — including the audio track's own packets. Getting this backwards does not raise a catchable error in FFmpeg-based tooling; it corrupts the container header and can crash (a native-level abort, not a Python exception) later, e.g. while flushing the video encoder at close. Found by this project's own audio tests, not by reading library documentation — treat it as load-bearing, not incidental ordering. |
| AAC encoder priming | AAC introduces a small, standard encoder delay (~1024–2112 samples, on the order of 25–50 ms at typical rates) at the start of a re-encoded track. This project doesn't attempt to trim it (documented, accepted V1 limitation, consistent with not correcting for it being standard practice across most tooling) — treat the first ~50 ms of any re-encoded audio track as approximate. |
