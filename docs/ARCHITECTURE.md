...
| **J** | *(User-requested follow-up, not part of the original nine.)* End-user packaging: no terminal/Python/CLI required on macOS/Windows; GUI demo/test mode without physical DMX hardware; Android without the user running `flutter create` themselves. | 🔄 **In progress.** Demo mode landed first, per the user's explicit priority ("I want to test the GUI now, before adding more features"): `dmxreplay.dmx.DemoDMXSource` (a deterministic, no-network synthetic DMX pattern), `Recorder.add_demo_source()`/`current_preview()` (feeds the identical `DMXEngine`/`DMXReplayWriter` path a real Art-Net/sACN source would, just without a socket), `dmxreplay.demo.demo_show_path()` (a bundled, cached-after-first-use demo `.dmxr` show with audio), a new `UniverseMonitor` Tk widget wiring Phase 9's previously-unused `dmxreplay.preview.rgb_led_pixels()` into both windows live, and a new `dmxreplay-gui` "Welcome to DMXReplay" launcher (`LauncherWindow`) so a user never needs to already know `dmxreplay-player-gui`/`dmxreplay-recorder-gui` exist as separate commands. Found and fixed a real bug while wiring the Recorder's demo option: `RecorderViewModel.add_demo_source()` initially called `Recorder.add_demo_source()` directly from the GUI thread, but that method starts an `asyncio` task internally and needs the background loop thread as its running-loop context — exactly the class of bug `async_bridge.AsyncLoopThread`'s own docstring warns against; fixed to dispatch via `call_soon`, with a regression test. All of it verified for real, not just written: 18 new headless tests (`tests/test_demo_source.py`, `tests/test_demo_show.py`, `tests/test_recorder_demo_source.py`, plus additions to the existing viewmodel test files) plus 5 new real Tkinter widget tests added to `tests_gui/` (bringing that suite to 9), run under a genuinely working `Xvfb` + Tkinter-enabled-Python (`3.12`, confirmed present in this environment) setup this session established for the first time — and the actual running GUI (launcher + Player playing the demo show + Recorder with its demo source active) was screenshotted for real via `ffmpeg x11grab` and sent to the user as visual proof, not just described. Full suite: 310 passed, 1 skipped. **Not yet done** (this row stays open until these land): a real macOS `.dmg` build step, a Windows installer (`.iss`/NSIS), and hand-authored Android `android/` project scaffolding so a user doesn't have to run `flutter create` themselves — `docs/DEMO_MODE.md` (new) covers what's done so far. |
