...
| **B** | Raspberry Pi ARM64/headless foundation: config-file loader for `RASPBERRY_PI.md` §14's proposed TOML shape, a real systemd unit, install script | ✅ **Done.** `dmxreplay.config.PlayerConfig` (real TOML loader, `tests/test_config_loader.py`) wired into `dmxreplay-play --config` (real end-to-end test over Art-Net, `tests/test_cli.py`). `packaging/systemd/dmxreplay-player.service` + `packaging/raspberrypi/install.sh` + example config — the systemd unit **verified for real** via `systemd-analyze verify` against an actual install at its expected path (`tests/test_packaging.py` re-checks this on every test run; it's also what caught a real misplaced-directive mistake in this unit file's first draft). `docs/RASPBERRY_PI_INSTALL.md` added. **Not verified**: actual Raspberry Pi hardware (none available here) — see that document's own hardware validation checklist (§7), which is what Phase H will work through. |
| **C** | Long-running commandable Player/Recorder service: the control surface `RASPBERRY_PI.md` §13 already identified as missing — a persistent process wrapping `Player`/`Recorder` that stays alive and accepts new commands after `play()` returns | ✅ **Done.** `dmxreplay.service.PlayerService`/`RecorderService` (`docs/API.md` §9) — plain asyncio objects, no network/GUI dependency, sequencing calls to `Player`/`Recorder` exactly like the CLI wrappers do, but staying alive to accept new commands. Includes a minimal `ShowLibrary` (list/resolve `.dmxr` files in a directory, with real path-traversal defense — `tests/test_show_library.py` includes a symlink-escape case) that `next_show()`/`previous_show()` use; upload/delete/rich metadata stays Phase G. 20 new tests, all real (Art-Net traffic, real `.dmxr` files), no mocking. |
| **D** | Control API (HTTP + WebSocket) over Phase C's service; `docs/MOBILE_API.md`; must never contain the real-time DMX loop itself | ✅ **Done.** `dmxreplay.control` (`docs/API.md` §10): `CommandRouter` (transport-agnostic, all 14 brief-listed commands), `ControlServer` (`aiohttp` — one HTTP `POST /api/v1/command` + 2 convenience `GET`s + one WebSocket endpoint broadcasting live `PlayerStatus` ~1/s), `ApiToken` (bearer-token auth, WS auth via first message not a query string). New `dmxreplay-server` CLI. Full wire protocol specified in `docs/MOBILE_API.md`. 27 new tests, all real (real HTTP/WebSocket traffic via `aiohttp`'s own test server, real Art-Net underneath, a real subprocess smoke test of the installed console script) — one real auth bug caught on first test run: the HTTP auth middleware was rejecting the WebSocket upgrade handshake itself, since a WS client sends no `Authorization` header by design (auth happens via the first WS message instead). |
| **E** | Raspberry Pi configuration + discovery: mDNS/Zeroconf advertisement, a lightweight local web config UI (network/Art-Net/sACN/playback/system settings) | ✅ **Done.** `dmxreplay.control.discovery` (`DeviceAdvertiser`/`discover_devices`, `zeroconf`) — real mDNS advertise+discover round trip verified in this environment (`tests/test_discovery.py`); full protocol in `docs/NETWORKING.md`. Local web config UI (`dmxreplay.control.webui`, plain HTML forms, no JS framework) wired into `ControlServer` at `/config`, `/config/restart`, `/config/shutdown`, `/config/logs` — playback/output settings apply live via the same `SET_CONFIG` command the JSON API uses; restart/shutdown work by choosing a process exit code that `Restart=on-failure` does or doesn't act on, not by shelling out to `systemctl`. New `packaging/systemd/dmxreplay-server.service` (the real production unit for this architecture — stays running as a server, unlike `dmxreplay-player.service`'s play-once-and-stop), `systemd-analyze verify`\-clean. `install.sh` now installs both units and the `[control]` extra (`aiohttp`+`zeroconf`). New `docs/NETWORKING.md`; `docs/RASPBERRY_PI_INSTALL.md`, `docs/API.md` §10, `docs/MOBILE_API.md` updated. 24 new tests, all real (mDNS, HTML escaping incl. a caught double-escape bug, live HTTP routes against `aiohttp`'s test server) — one more real bug caught by testing, not inspection: the log ring buffer's logger had no level set, so `/config/logs` silently showed nothing (Python loggers default to `WARNING`, filtering out the `INFO` status messages before they ever reached the buffer). |
| **F** | Mobile remote controller: cross-platform app (Flutter recommended, §6) talking only to Phase D's API — browse/select/play/pause/seek/next/previous/record/status, never on-device DMX decode | ⚠️ **Code complete, not compiled/run — see `docs/MOBILE.md` §8.** A real, production-structured Flutter project (`mobile/`) built against Phase D's actual API: `DmxReplayRestClient`/`DmxReplayWebSocketClient` (REST for every command, WebSocket for the status broadcast only — the wire protocol has no request-id field, documented rationale in the WS client's own doc comment), mDNS discovery (`multicast_dns`, mirroring `dmxreplay.control.discovery`'s advertised TXT record), `ChangeNotifier`-based state (`ConnectionController`/`PlayerController`/`RecorderController`, no extra state-management package), and 5 screens (discovery/pairing, player/transport/timeline, show library, recorder, output+network settings) plus shared status/error widgets. Independent of the Python package (no shared build). One real API gap found while building the client — no way to poll live recorder status without restarting the recording — fixed at the source: `GET_RECORDER_STATUS` added to `dmxreplay.control.CommandRouter` (2 new tests in `tests/test_control_router.py`) and documented in `docs/MOBILE_API.md`. **This environment has no Flutter SDK** — the Dart code has not been compiled, analyzed, or run; see `docs/MOBILE.md`/`mobile/README.md` for the exact validation status and the commands (`flutter create`/`pub get`/`analyze`/`test`/`run`) a developer with Flutter installed should run next. Dart tests are written (`mobile/test/`: model parsing, REST client request/response shapes via `package:http/testing.dart`'s real `MockClient`, TXT-record parsing, two widget tests) but likewise not executed here. |
| **G** | Show management + file transfer: show-library abstraction, browse/select/delete/info via the Control API, upload from client to Pi | ✅ **Done.** `ShowLibrary.delete()`/`save()` (atomic write via temp file + `os.replace()`, path-traversal-safe like `resolve()`) underneath two new `PlayerService`/`CommandRouter` commands, `GET_SHOW_INFO` (real manifest metadata — duration/fps/encoding/universe count/audio/video/file size — for any show, not just the loaded one) and `DELETE_SHOW` (refuses to delete the currently-*playing* show), plus one new HTTP-only endpoint outside the JSON command protocol, `PUT /api/v1/shows/{name}` (raw `.dmxr` bytes, 512 MiB cap, re-opens the uploaded file to confirm it's a real DMXReplay container before accepting it — deletes it again on any failure). Found and fixed a real pre-existing gap while adding these: `_dispatch_to_response`/`_handle_ws_message` only caught `CommandError`, so exceptions the underlying `Player`/`ShowLibrary` calls actually raise (`ShowNotFoundError` and friends, all plain `ValueError`/`OSError`/`RuntimeError`) were reaching clients as an unhandled 500 instead of the 409 `docs/MOBILE_API.md` §7 has always documented — both handlers now catch the broader set, with a regression test. Mobile client updated to match: `deleteShow()`/`getShowInfo()`/`uploadShowBytes()` on `DmxReplayRestClient`, wired into `ShowsScreen` (a per-show info dialog and delete-with-confirmation) — upload has no picker UI yet (needs a file-picker plugin this project doesn't depend on, documented as a follow-up in `docs/MOBILE.md`). 27 new Python tests (all real: real `.dmxr` files, real filesystem operations, real HTTP round trips via `aiohttp`'s test server), full suite 292 passed/1 skipped. `docs/API.md`, `docs/MOBILE_API.md`, `docs/MOBILE.md` updated. |
| **H** | Performance / hardware validation: the full matrix (1/10/50/128 universes × DMX/+audio/+video/+audio+video) on real Pi 4/5 where possible; a hardware validation checklist where it isn't | ⚠️ **Dev-machine measurements done; real Pi still required — see `docs/PERFORMANCE.md`.** New `benchmark/realtime_playback_benchmark.py` runs the real `Player`, real-time paced (not §4's unthrottled throughput measurement), against a real Art-Net loopback listener, at {1, 50} universes × {dmx, audio, video, audio_video} — measuring both actual packet-timing jitter (mean well under 3.5 ms, max under one frame period at every point) and the incremental CPU cost of real audio (AAC, muxed via `DMXReplayWriter`) and real external video (H.264, via `Player.load_external_video()`) decode, neither of which shows up in §4's DMX-only throughput number since both run on separate PyAV streams off the same `Timeline`. Finding: video decode costs ~4× the DMX-only CPU fraction at both universe counts measured; audio costs almost nothing extra. A real bug in the benchmark itself was caught by the numbers looking implausible (not by inspection): filtering received packets by the raw 4-bit Art-Net `universe` field alone silently over-counted "one representative universe"'s arrivals ~4× past 16 universes, since that field repeats — fixed to match the full `(net, subnet, universe)` triple. `docs/RASPBERRY_PI_INSTALL.md` §7's hardware validation checklist (already existed) is unchanged in substance, cross-referenced from `docs/PERFORMANCE.md`, and gained two new items this phase's findings motivate directly (confirming the measured 50-universe `+video` cost at the already-tight 128-universe/Pi-4 scale point; confirming jitter on real hardware is at least as good as this shared cloud VM's). No physical Raspberry Pi exists in this environment — every number here is honestly labeled as a dev-machine measurement, same caveat `docs/RASPBERRY_PI.md` §5 already established for its own extrapolation. |
| **I** | Final packaging + documentation pass across all new docs (`ARCHITECTURE.md`, `API.md`, `RASPBERRY_PI.md`, `RASPBERRY_PI_INSTALL.md`, `MOBILE.md`, `MOBILE_API.md`, `NETWORKING.md`, `BUILD_AND_DISTRIBUTION.md`) | ✅ **Done.** Every doc this row lists was already kept current phase-by-phase (this project's established convention, not a batch operation deferred to the end) — this pass instead swept for and fixed what incremental updates miss: stale *predictive* statements written before the thing they predicted was actually built. Found two: `docs/API.md` §10 and `docs/NETWORKING.md` §3 both said the mobile app "will use its platform's own native mDNS APIs (`NSNetService`/`NsdManager`)" — written during the Phase E audit, before Phase F was actually implemented using the Flutter team's cross-platform `multicast_dns` package instead (a real, deliberate choice, `docs/MOBILE.md` §5), never corrected until now. Also fixed: `docs/API.md` §10's local-web-config-UI paragraph phrased Phase F as hypothetical ("once it exists") after it had already landed. `docs/BUILD_AND_DISTRIBUTION.md` gained a one-line pointer to `docs/MOBILE.md` for the mobile app's entirely separate Flutter build process (this document only ever covered the Python desktop apps). `docs/RASPBERRY_PI.md` §10 gained a cross-reference to `docs/PERFORMANCE.md`'s new audio/video decode cost measurement. All shell scripts (`packaging/*.sh`, `benchmark/*.sh`) re-verified with `bash -n`; full test suite re-run clean (292 passed, 1 skipped) as the final packaging check. `README.md`'s top status line and phase table updated to reflect all nine phases landed, with Phase F/H's honest hardware-pending caveats carried through rather than smoothed over. |

| **J** | *(User-requested follow-up, not part of the original nine.)* End-user packaging: no terminal/Python/CLI required on macOS/Windows; GUI demo/test mode without physical DMX hardware; Android without the user running `flutter create` themselves. | ✅ **Done, with the usual real-hardware caveats carried through honestly.** Demo mode landed first, per the user's explicit priority ("I want to test the GUI now, before adding more features"): `dmxreplay.dmx.DemoDMXSource`, `Recorder.add_demo_source()`/`current_preview()`, `dmxreplay.demo.demo_show_path()` (a bundled, cached demo `.dmxr` show with audio), a new `UniverseMonitor` Tk widget wiring Phase 9's previously-unused `rgb_led_pixels()` into both windows live, and a new `dmxreplay-gui` "Welcome to DMXReplay" launcher (`LauncherWindow`) -- full detail in `docs/DEMO_MODE.md`. Found and fixed a real bug in the process (`RecorderViewModel.add_demo_source()` calling an asyncio-task-starting method directly on the GUI thread instead of via `call_soon` -- exactly what `async_bridge.AsyncLoopThread`'s own docstring warns against), with a regression test. Verified for real: 18 new headless tests, 5 new Tkinter widget tests (9 total in `tests_gui/`, run under a genuinely working `Xvfb`+Tkinter-enabled-Python `3.12` setup established this session), and the actual running GUI screenshotted via `ffmpeg x11grab` and sent to the user. **Packaging**: new `dmxreplay_gui.spec` (the single launcher app end users should run) -- built *and run standalone* on Linux (no dev venv, just the packaged binary), screenshotted the same way (`docs/BUILD_AND_DISTRIBUTION.md` §3.1); `packaging/windows/dmxreplay.iss` (a real Inno Setup script, wired into `build_windows.ps1` to run automatically if `ISCC.exe` is present) producing `DMXReplay-Setup.exe`; `build_macos.sh` now stages and runs a real `hdiutil create` producing `DMXReplay.dmg`. Neither the Windows installer nor the macOS build/dmg step has been run on real hardware (none available here) -- same caveat as every other Windows/macOS claim in this project. **Android**: `mobile/android/` is now a real, committed, hand-authored Flutter Android project -- including a **genuine Gradle wrapper** (`gradle-wrapper.jar` actually produced by this environment's own Gradle 8.14.3 via its `wrapper` task, not a guessed binary) -- so a developer runs `flutter pub get`/`flutter build apk` directly, no `flutter create --platforms=android .` step needed. Every XML file was verified well-formed (`xml.etree.ElementTree` caught and fixed several double-hyphen-in-comment errors before commit); the Groovy Gradle build scripts themselves were not evaluated by Gradle here (needs the real Flutter SDK's plugin-loader plus an Android SDK, neither present) -- full status in `mobile/README.md`/`docs/MOBILE.md` §8. Full Python test suite: 310 passed, 1 skipped throughout. |
