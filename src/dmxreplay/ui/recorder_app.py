...
class RecorderWindow:
    ...
    def _on_add_source(self) -> None:
        self.vm.add_source(self.protocol_var.get(), self.interface_entry.get() or "0.0.0.0")
    ...
    def _render(self, snap: RecorderSnapshot) -> None:
        ...
        self.network_status_label.config(
            text=f"Network: listening, {snap.status.universe_count} universe(s) seen"
            if snap.universes else "Network: not listening"
        )
